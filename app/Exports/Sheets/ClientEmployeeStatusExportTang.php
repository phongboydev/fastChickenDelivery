...
class ClientEmployeeStatusExportTang implements FromView, WithTitle, WithEvents
{
    ...
    public function view(): View
    {
        ...
        $employees = ClientEmployee::select('full_name', 'sex', 'date_of_birth', 'type_of_employment_contract', 'education_level', 'position', 'official_contract_signing_date')
            ...
            ->whereBetween('official_contract_signing_date', [$this->from_date, $this->to_date])->get();

        if( !empty($employees) ) {
            $this->total_list = count($employees);
        }

        return view('exports.client-employee-status-tang', [
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle('A3:N4')->getAlignment()->applyFromArray([
                    'vertical' => 'center',
                    'horizontal' => 'center'
                ]);

                $cellRange = 'A3:N' . (4 + $this->total_list);
