 ...
 */
class ClientEmployeeEarlyLateRequest extends Model
{
    use Concerns\UsesUuid, SoftDeletes, LogsActivity, HasAssignment, BelongsToThrough;

    protected static $logAttributes = ['*'];

    protected $table = 'client_employee_early_late_requests';
     ...
     * @var array
     */
    protected $fillable = ['client_employee_id', 'approved_by', 'start_time', 'end_time', 'duration', 'type', 'reason', 'status', 'approved_date', 'approved_comment', 'deleted_at', 'created_at', 'updated_at'];
        ...
        if (!$user->isInternalUser()) {
            ...
            switch ($role) {
                ...
                case Constant::ROLE_CLIENT_STAFF:
                    ...
                    logger("ClientEmployeeEarlyLateRequest::scopeAuthUserAccessible Customer query");
                    logger($query->toSql());
                    return $query;
            }
        } else {
